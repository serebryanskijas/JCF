...
public class WiseCounting {
    public static void main(String[] args) {
        ...
        courtiers.add(new Courtier("Tretiy"));
        Verbal verbal = new Verbal("Verbal");
        emperor.asks(0);
       /* for (int i =0; i<courtiers.size(); i++) аналогичная запись
            courtiers.get(i).thinks();*/
        /*for(Courtier courtier: courtiers) аналогичная запись
            courtier.thinks();*/
        courtiers.forEach(Courtier::thinks); //аналогичная запись
        verbal.enters();
        verbal.asks();
        emperor.asks(0);
        verbal.replace();
